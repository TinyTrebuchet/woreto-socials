...
public class FBDaoService {
    ...
    public List<FBPage> fetchAllPages(){
        ...
        String sql = "SELECT * FROM fb_page ORDER BY lastPosted DESC;";
    ...
    public void updateLastPosted(FBGroup group) {
        long currentTime = System.currentTimeMillis();
        String sql = "UPDATE fb_group SET lastPosted = ? WHERE id = ?;";
        try (Connection conn = DriverManager.getConnection(DB_URL);
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setLong(1, currentTime);
            ...
            int affectedRows = stmt.executeUpdate();
            if (affectedRows == 0) {
                LOGGER.error("No group updated with id {}", group.getId());
            }
        } catch (Exception e) {
            LOGGER.error("Error saving facebook group {}", group.getId(), e);
        }
    }
    ...
    public FBPagePost fetchLatestPagePost() {
        String sql = "SELECT * FROM fb_page_post ORDER BY createdTime DESC LIMIT 1;";
        try (Connection conn = DriverManager.getConnection(DB_URL);
             Statement stmt = conn.createStatement()) {
            ResultSet rs = stmt.executeQuery(sql);
            rs.next();
